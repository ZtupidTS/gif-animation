...
Global $hGIF = GUICtrlCreatePic("", $iPosX, $iPosY, $aGIFDimension[0], $aGIFDimension[1]);_GUICtrlCreateGIF($sFile, "", $iPosX, $iPosY)
_GUICtrlSetGIF($hGIF, $sFile)
If @extended Then
	GUICtrlSetData($hButton1, "Not animated")
	GUICtrlSetState($hButton1, $GUI_DISABLE)
EndIf
GUICtrlSetTip($hGIF, $aGIFDimension[0] & " x " & $aGIFDimension[1], StringRegExpReplace($sFile, ".*\\", ""), 1)
...
;~ 	$hGIF = _GUICtrlCreateGIF($aGIFFilesArray[$iCurrentPos], "", $iPosX, $iPosY)
	GUICtrlSetState($hGIF, $GUI_HIDE)
	GUICtrlSetPos($hGIF, $iPosX, $iPosY, $aGIFDimension[0], $aGIFDimension[1])
	_GUICtrlSetGIF($hGIF, $aGIFFilesArray[$iCurrentPos])
	If @extended Then
		GUICtrlSetData($hButton1, "Not animated")
		GUICtrlSetState($hButton1, $GUI_DISABLE)
	Else
	...
	GUICtrlSetTip($hGIF, $aGIFDimension[0] & " x " & $aGIFDimension[1], StringRegExpReplace($aGIFFilesArray[$iCurrentPos], ".*\\", ""), 1)
EndFunc   ;==>_Back
...
Func _aa()
	Local $pGIF = _GIF_GetGIFAssoc($hGIF)
	; Read $pGIF (shorter version - only handles and memory pointers)
	Local $tGIF = DllStructCreate("handle GIFThread;" & _
			"ptr CodeBuffer;", _
			$pGIF)

	Local $tColorBuffer = DllStructCreate("dword Color", DllStructGetData($tGIF, "CodeBuffer") + 163);+ (234-163)*@AutoItX64)

	ConsoleWrite("0x" & Hex(DllStructGetData($tColorBuffer, "Color")) & @CRLF)
